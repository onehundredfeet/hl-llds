...
";
	...
	static var options = { idlFile : "src/llds.idl", nativeLib : "llds", outputDir : "src", includeCode : INCLUDE, autoGC : true };
	...
	public static function getFiles() {
		var prj = new haxe.xml.Access(Xml.parse(sys.io.File.getContent("llds.vcxproj.filters")).firstElement());
		var sources = [];
	...
	public static function generateJs() {
		...
		var params = ["-O"+(debug?0:3), "--llvm-lto", "1", "-I", "../../include/llds/src"];
		for( d in defines ) {
			params.push("-s");
			params.push(d);
		}
